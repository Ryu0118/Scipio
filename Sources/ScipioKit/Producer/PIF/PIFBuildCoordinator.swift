...
struct PIFBuildCoordinator {
    ...
    func executeBuildForMultipleTargets(
        ...
        sdk: SDK,
        buildOptions: BuildOptions,
        xcBuildClient: XCParallelBuildClient
    ) async throws {
        let toolchain = try await makeToolchain(for: sdk)
        let buildParameters = await buildParametersGenerator.generate(from: buildOptions, toolchain: toolchain)
    ...
    /// Create XCFramework from built frameworks (single target)
    ...
    /// Create XCFramework from built frameworks (parallel version)
    func createXCFramework(
        buildProduct: BuildProduct,
        buildOptions: BuildOptions,
        sdks: Set<SDK>,
